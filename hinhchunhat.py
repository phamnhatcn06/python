import turtle
import math
# Defined Variable
penDraw = turtle.Turtle()
penDraw.speed(10)
penDraw.pensize(3)  # kích cỡ


def setPosition(longPos, latPos):
    penDraw.up()
    penDraw.goto(longPos, latPos)
    penDraw.down()


def drawSquare(width, height, colorBack, borderWidth):
    penDraw.pensize(borderWidth)
    penDraw.pendown()  # tạo con trỏ
    penDraw.color(colorBack)
    penDraw.pencolor("black")
    penDraw.begin_fill()
    for counter in range(1, 3):  # đây là một vòng lặp (loop)
        penDraw.forward(width)
        penDraw.right(90)
        penDraw.forward(height)
        penDraw.right(90)
    penDraw.end_fill()
    penDraw.penup()


if __name__ == '__main__':
    w = float(input('Nhập vào chieu dai hinh chu nhat:'))
    h = float(input('Nhập vào chieu rong hinh chu nhat:'))
    chuvi = (w+h) * 2
    dientich = w*h
    drawSquare(w, h, "red", 3)
    setPosition(0, -h-100)
    penDraw.color('red')
    penDraw.pencolor("black")
    penDraw.write("Chu vi hinh chu nhat la {} \r\n Dien tich la {}".format(
        chuvi, dientich), font=("Arial", 16, "bold"))
    turtle.done()
